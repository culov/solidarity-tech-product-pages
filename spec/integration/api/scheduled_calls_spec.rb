...
describe 'Scheduled Calls API', type: :request, swagger_doc: 'v1/swagger.yaml' do
  path '/scheduled_calls' do
    get 'Lists scheduled calls' do
      ...
      parameter name: :user_id, in: :query, description: 'User ID to filter scheduled calls related to a specific user.', required: false, schema: { type: :integer }
      ...
      response '200', 'scheduled calls listed' do
  ...
  path '/scheduled_calls/{id}' do
    get 'Shows a single scheduled call' do
      tags 'Scheduled Calls'
      produces 'application/json'
      parameter name: :id, in: :path, type: :integer, required: true, description: 'ID of the scheduled call to retrieve'

      # TODO: Add calls to DB for make this to pass.
      # response '200', 'scheduled call found' do
      #   it 'returns a 200 response' do |example|
      ...
      response '404', 'scheduled call not found' do
