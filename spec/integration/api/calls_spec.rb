...
describe 'Calls API', type: :request, swagger_doc: 'v1/swagger.yaml' do

  path '/calls' do

    get 'Retrieves all calls' do
      tags 'Calls'
      produces 'application/json'
      parameter name: :user_id, in: :query, description: 'User ID to filter calls related to a specific user.', required: false, schema: { type: :integer }
      ...
      response '200', 'successful' do
        let(:user_id) { User.where(root_organization_id: 1).limit(1).first.id }
        let(:_limit) { 20 }
        let(:_offset) { 0 }
