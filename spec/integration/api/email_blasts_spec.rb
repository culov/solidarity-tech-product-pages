...
describe 'Email Blasts API', type: :request, swagger_doc: 'v1/swagger.yaml' do
  path '/email_blasts' do
    get 'Lists email blasts' do
      tags 'Email Blasts'
      ...
      response '200', 'email blasts listed' do
  ...
  path '/email_blasts/{id}' do
    get 'Shows a single email blast' do
      tags 'Email Blasts'
      produces 'application/json'
      parameter name: :id, in: :path, type: :integer, required: true, description: 'ID of the email blast to retrieve'

      response '200', 'email blast found' do
        it 'returns a 200 response' do |example|
      ...
      response '404', 'email blast not found' do
