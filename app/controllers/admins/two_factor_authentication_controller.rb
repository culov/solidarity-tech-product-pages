class Admins::TwoFactorAuthenticationController < Devise::TwoFactorAuthenticationController
  ...
  def after_two_factor_success_for(resource)
    ...
    if resource.urs && resource.urs.solidarity_user && resource.phone_number == resource.urs.solidarity_user.email && resource.urs.solidarity_user.email_verified != true

      resource.urs.solidarity_user.update_attribute(:email_verified, true)
  ...
  def after_two_factor_success_path_for(resource)
    stored_location_for(resource_name) || "/people"
  end
  ...
  def after_two_factor_fail_for(resource)
    ...
    if resource.max_login_attempts?
      sign_out(resource)
      set_flash_message :alert, :max_login_attempts_reached
      redirect_to "/admins/two_factor_authentication"
    else
      # render :show
  ...
  def prepare_and_validate
    if resource.nil?
      redirect_to 'https://dashboard.solidarity.tech/login' and return
    end
